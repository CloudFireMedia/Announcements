...
//
//  var archiveDoc = DocumentApp.getActiveDocument();
//
//  //firstly make sure we can access the archive doc
//  if( ! Config.get('ANNOUNCEMENTS_ARCHIVE_ID')) throw 'Archive Setup is not configured.  Archiving cannot run';
//  try{ var archiveDoc = DocumentApp.openById(Config.get('ANNOUNCEMENTS_ARCHIVE_ID')); }
//  catch(e){throw 'Error accessing archive document. ('+Config.get('ANNOUNCEMENTS_ARCHIVE_ID')+')';};
